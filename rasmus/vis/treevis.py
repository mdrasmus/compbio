...
class TreeViewer (sumtree.SumTree):
    ...
    def setup_recon(self, recon=None):
        ...
        if recon is None and self.stree and self.gene2species:
            self.recon = phylo.reconcile(
                self.tree, self.stree, self.gene2species)
        else:
            self.recon = recon
    ...
    def show(self):
        ...
        if self.bar is None:
            ...
            self.bar.add_item(hud.MenuItem("gene mode (g)",
                                           lambda: self.set_mode("gene")))
            self.bar.add_item(hud.MenuItem("events mode (e)", lambda:
                                           self.set_mode("events")))
            self.bar.add_item(hud.MenuItem("reroot mode (r)",
                                           lambda: self.set_mode("reroot")))
            self.bar.add_item(hud.MenuItem("swap mode (s,S)",
                                           lambda: self.set_mode("swap")))
    ...
    def draw_events(self):
        ...
        # draw losses
        losses_per_branch = util.hist_dict(
            [node for node, schild in self.losses])
