...
The current intended usage is either as module within other projects:
    ...
    from svglib import svg2rlg, svg2pdf
    drawing = svg2rlg('foo.svg') # then use drawing as you like...
    svg2pdf('foo.svg')           # this will create a file 'foo.pdf'
  
or from the command-line where right now it is usable as an SVG to PDF
converting tool (which should also handle SVG files compressed with
gzip and extension .svgz).

This is really beta code, so be careful!
...
import string, sys, os, glob, types, re, operator, gzip
...
__version__ = '0.5'
__license__ = '90 day beta evaluation version for personal use'
...
class AttributeConverter:
    ...
    def parseMultiAttributes(self, line):
        ...
        attrs = map(string.strip, attrs)
        attrs = filter(lambda a:len(a)>0, attrs)

        newAttrs = {}
        for a in attrs:
            k, v = string.split(a, ':')
            k, v = map(string.strip, (k, v))
            newAttrs[k] = v
    ...
    def convertTransform(self, svgAttr):
        ...
        for i in range(0, len(brackets), 2):
            ...
            subline = string.strip(subline)
            subline = string.replace(subline, ',', ' ')
...
class Svg2RlgAttributeConverter(AttributeConverter):
    ...
    def convertLength(self, svgAttr, percentOf=100):
        ...
        newSize = text[:]
        for u in string.split('em ex px '):
            if string.find(newSize, u) >= 0:
                if LOGMESSAGES:
                    print "Ignoring unit: %s" % u
                newSize = string.replace(newSize, u, '')

        newSize = string.strip(newSize)
    ...
    def convertColor(self, svgAttr):
        ...
        if text in string.split(predefined):
        ...
        elif text[:3] == 'rgb' and string.find(text, '%') < 0:
            t = text[:][3:]
            t = string.replace(t, '%', '')
            tup = eval(t)
...
class SvgRenderer:
    ...
    def render(self, node, parent=None):
        ...
        if name == 'svg':
            ...
            methodName = 'convert'+string.upper(name[0])+name[1:]
            n = NodeTracker(node)
    ...
    def printUnusedAttributes(self, node, n):
        ...
        if self.verbose and unusedAttrs:
            format = "%s-Unused: %s"
            args = ('  '*(self.level+1), string.join(unusedAttrs, ', '))
            #if not self.logFile:
            #    print format % args
            #else:
            #    self.logFile.write((format+"\n") % args)
    ...
    def renderSvg(self, node):
        ...
        if not width or not height:
            print "Aborting! SVG node has no 'width' and/or 'height' attribute."
            sys.exit(1)
        self.drawing = Drawing(width, height)
        viewBox = getAttr('viewBox')
        if viewBox:
            if LOGMESSAGES:
                print "Ignoring attribute: svg.viewBox"
        return self.drawing
...
class SvgShapeConverter:
    ...
    def getHandledShapes(self):
        ...
        shapeNames = map(lambda k:string.lower(k), keys)
...
class Svg2RlgShapeConverter(SvgShapeConverter):
    ...
    def convertText(self, node):
        ...
        for c in node.childNodes:
            ...
            if c.nodeType == c.TEXT_NODE:
                ...
                try:
                    tx = string.join(map(chr, map(ord, frags[-1])), '')
                except ValueError:
                    tx = 'Unicode'
            ...
            try:
                text = string.join(map(chr, map(ord, frags[-1])), '')
            except ValueError:
                text = 'Unicode'
    ...
    def applyStyleOnShape(self, shape, *nodes):
        ...
        mappingN = (('fill', 'fillColor', 'convertColor', 'none'), 
        ...
        mappingF = (('font-family', 'fontName', 'convertFontFamily', 'Helvetica'),
...
def svg2pdf(path):
    ...
    # save as PDF file
    if drawing:
        base = os.path.splitext(path)[0] + '.pdf'
...
if __name__ == '__main__':
    ...
    if len(argv) == 1 or '-h' in argv or '-help' in argv or '--help' in argv:
        format = "%s (v. %s) - an experimental SVG to RLG/PDF converter"
        print format % (prog, __version__)
        format = "Usage: python %s [--help] <file1.svg(z)> [<file2.svg(z)> ...]"
        print format % prog
        sys.exit(0)
    ...
    for path in paths:
        ...
        if string.lower(os.path.splitext(path)[1]) == '.svgz':
